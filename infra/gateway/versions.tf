terraform {
  ...
  # S3 backend below (see env/gateway.dev.backend.hcl.example).
  ...
  required_providers {
    ...
    # Used to generate the CloudFront origin-verify shared secret.
    random = {
  ...
  # -backend-config=../env/gateway.<env>.backend.hcl (never hard-coded here).
  backend "s3" {}
}
